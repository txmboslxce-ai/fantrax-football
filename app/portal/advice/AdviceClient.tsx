...
function tieredBadgeClass(value: number, values: number[]): string {
  if (values.length === 0 || values.every((item) => item === 0)) return "bg-slate-100 text-slate-500";

  const rank = values.filter((item) => item > value).length + 1;
  const fraction = rank / values.length;
  if (fraction <= 0.2) return "bg-green-100 text-green-800";
  if (fraction <= 0.4) return "bg-lime-100 text-lime-800";
  if (fraction <= 0.6) return "bg-yellow-100 text-yellow-800";
  if (fraction <= 0.8) return "bg-orange-100 text-orange-800";
...
export default function AdviceClient({ players, leagueRoster }: Props) {
  ...
  const oppStatValues = useMemo(() => players.map((row) => row.oppStats[selectedStat]), [players, selectedStat]);
  const playerStatValues = useMemo(() => players.map((row) => row.playerStats[selectedStat]), [players, selectedStat]);
        ...
        <div className="grid grid-cols-2 gap-2 text-xs md:flex md:flex-nowrap md:items-end md:gap-2">
          ...
          <label className="col-span-2 space-y-1 md:col-span-1 md:shrink-0">
            ...
            <select
              ...
              className="w-full rounded border border-slate-300 bg-white px-2 py-1 text-xs text-brand-dark focus:border-brand-green focus:outline-none md:w-44"
            >
          ...
          <label className="space-y-1 md:shrink-0">
            ...
            <input
              ...
              placeholder="Player"
              className="w-full rounded border border-slate-300 bg-white px-2 py-1 text-xs text-brand-dark placeholder:text-slate-400 focus:border-brand-green focus:outline-none md:w-36"
            />
          </label>
          ...
          {/* Min starts */}
          <label className="space-y-1 md:shrink-0">
            <span className="block font-semibold uppercase tracking-wide text-slate-600">Min starts</span>
            <input
              type="number"
              min={0}
              value={minStarts}
          ...
          <div className="space-y-1 md:shrink-0">
            <span className="block font-semibold uppercase tracking-wide text-slate-600">Fixture</span>
            <div className="flex flex-nowrap gap-1">
  ...
  return (
    <div className="space-y-3">
      ...
      <div className={mobileFiltersOpen
        ? "fixed inset-0 z-50 space-y-3 overflow-y-auto bg-white p-4 pb-24 md:static md:inset-auto md:z-auto md:overflow-visible md:bg-transparent md:p-0"
        : "hidden md:block md:space-y-3"}
      >
        {mobileFiltersOpen ? (
          <div className="flex items-center justify-between md:hidden">
            <span className="text-sm font-bold uppercase tracking-widest text-brand-dark">Filters</span>
            <button
              type="button"
              onClick={() => setMobileFiltersOpen(false)}
              className="rounded-full border border-slate-300 bg-white px-4 py-1.5 text-sm font-semibold text-brand-dark hover:bg-slate-50"
            >
              Done
            </button>
          </div>
      ...
      >
        {hasActiveFilters ? (
          <span className="absolute -right-1 -top-1 h-3 w-3 rounded-full bg-white ring-2 ring-brand-green" aria-hidden="true" />
        ) : null}
  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs font-bold text-brand-dark">{value.toFixed(digits)}</span>
      </div>
      <input
        type="range"
        min={0}
