...
async function main() {
  ...
  for (let i = 0; i < toApply.length; i += BATCH) {
    const batch = toApply.slice(i, i + BATCH).map((r) => ({
      id: r.our_player_id,
      sofascore_id: parseInt(r.sofascore_id, 10),
    }));

    const invalidBatch = batch.filter((r) => isNaN(r.sofascore_id));
    if (invalidBatch.length > 0) {
      console.warn(
        `  Warning: ${invalidBatch.length} rows in batch have non-numeric sofascore_id — skipping them.`
      );
      const validBatch = batch.filter((r) => !isNaN(r.sofascore_id));
      if (validBatch.length === 0) continue;

      const { error } = await supabase.from("players").upsert(validBatch, { onConflict: "id" });
      if (error) {
        console.error(`  Batch error at offset ${i}:`, error.message);
        errors++;
      } else {
        written += validBatch.length;
      }
    } else {
